    ...
    **kwargs,
    ...
    if not _FLASH_ATTN_AVAILABLE or flash_attn_varlen_func is None:
        raise ImportError(
            ...
            "Install `flash-attn` or use `attn_implementation='sdpa'`/`'eager'`."
        )
    ...
    def _to_cu_seqlens(mask: Optional[torch.Tensor | dict]) -> torch.Tensor:
        ...
        if isinstance(mask, dict):
            cu_q = mask.get("cu_seqlens_q", None)
            if cu_q is None:
                cu_q = mask.get("cu_seqlens", None)
            if cu_q is not None:
...
def replace_qwen2_vl_attention_class():
    if not _FLASH_ATTN_AVAILABLE:
        raise ImportError(
            "flash-attn is not available; cannot enable the Qwen2/Qwen2.5 varlen FlashAttention2 patch."
        )
    import transformers
