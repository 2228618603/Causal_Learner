...
def create_pointcloud_adapter(
    ...
    # IMPORTANT:
    # We only need the PointBERT backbone + point_proj here; creating a full Llama stack is wasteful.
    # Build a minimal LlamaConfig (0 layers + tiny vocab) to keep memory footprint small.
    cfg = PointLLMConfig(
        vocab_size=8,
        hidden_size=pll_hidden_for_build,
        intermediate_size=max(int(pll_hidden_for_build) * 4, 16),
        num_hidden_layers=0,
    ...
    if proj_out_dim != hidden_size:
        model.align_mlp = nn.Linear(proj_out_dim, hidden_size).to(device)
    else:
    ...
    # IMPORTANT: PointLLMLlamaModel inherits a (minimal) LlamaModel, but in this repo we only use
    # `point_backbone`, `point_proj`, and `align_mlp`. Freeze everything by default to avoid
    # unused-trainable parameters under DDP (the training script will selectively unfreeze).
