...
- Script-driven generators (two-stage legacy): `two_satge_old/mani_*video*.py` (manipulation), `two_satge_old/nav_*` (navigation), `two_satge_old/generate_phyplan_api*.py` (API-based generators).
- Post-processing helpers (two-stage legacy): `two_satge_old/extract_*_segments.py`, `two_satge_old/post_select_keyframes.py`, `two_satge_old/video_duration_classifier*.py`.
- Task specs/notes (two-stage legacy): `two_satge_old/mani_*_tasks_plan*.md`, with `two_satge_old/mani_longvideo_tasks_plan_final.md` as the canonical long-video task set.
- Three-stage long-video pipeline: `three_stage/` (separate implementation; spec in `three_stage/THREE_STAGE_PIPELINE_WITH_EXAMPLES.md`).
...
## `two_satge_old/mani_longvideo_tasks_plan_final.md`
...
- Long-video generator: `python two_satge_old/mani_longvideo.py`
- Three-stage pipeline: `python three_stage/pipeline.py --input-video /abs/path/video.mp4`
- Segment export utilities: `python two_satge_old/extract_last_frame_segments.py`, `python two_satge_old/extract_cumulative_last_frame_segments.py`
