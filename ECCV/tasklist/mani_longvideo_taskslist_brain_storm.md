...
本任务集假设每个 item 有一份标注 JSON（文件名仅示例；数据可来自二阶段或三阶段等不同生成管线），其字段结构与 `ECCV/two_stage/mani_longvideo.py` 的 dataclass 以及 `ECCV/three_stage/prompts.py` 的最终产物 schema 保持一致。
...
### 1.4 Prompt Context 最小化原则（面向 QA-only）

- Context 只给“完成该 QA 所必需”的信息：多数关键帧任务只需 `step_goal`；全局/长时序规划任务才需要 `high_level_goal`。
- 不在 Context 中直接提供该任务的 label 字段（避免泄漏）；若 label 由多字段组合（如 Task_10），Context 仍只给必要的 `step_goal`，其余由证据推断。
- 涉及进度边界（`video_prefix`）的任务，优先给 `Last completed step_goal` 来定义“下一步”的语义，而不是暴露 `step_id/frame_index`。
- 跨步依赖类任务，Context 只给 1 条 effect + 1 条 precondition（原子陈述），避免全量列表引入冗余与歧义。
- 失败反思闭环任务，Context 至少包含失败发生的 `step_goal` + `failure reason` + `recovery strategy`；`high_level_goal` 仅在需要选择下一步/重规划时提供。
...
- `Multimodal_input` 行里的括号参数（如 `step_id/frame_index/prefix_end_step`）仅用于数据对齐的 meta，不进入模型看到的 Q 文本；Q 中显式写出的 `Context: ...` 才是给模型的上下文。
...
- **字段（JSONPath）**：
  - `high_level_goal`（可选上下文；默认不提供，仅当 clip 过短/歧义时提供）
...
- **任务说明**：在单张关键帧上做“此刻是否可执行”的物理可行性判别。为避免模型凭空编造前置条件，prompt **显式给定** 1 条空间前置条件与 1 条可供性前置条件（从 `critical_frames[0]` 的 frame-level preconditions 中抽取的原子陈述）；模型只需判断它们在该帧上是 `satisfied/violated/not directly observable`，并给出整体 `feasible / not feasible` 结论（1 句英文）。
- **与前置条件描述类任务的区别**：前置条件类任务更偏“描述/对齐具体前置条件条目”，本任务对 `step_goal` 做整体可行性判断，并要求给出一条空间与一条可供性依据，更贴近执行决策。
- **字段（JSONPath）**：
  - 上下文：`steps[i].step_goal`
  - 输入（写入 Q 里）：从 `steps[i].critical_frames[0].causal_chain.causal_precondition_on_spatial` 拆分抽取 1 条空间原子前置条件；从 `steps[i].critical_frames[0].causal_chain.causal_precondition_on_affordance` 拆分抽取 1 条可供性原子前置条件
  - label：对两条给定前置条件分别判定 `satisfied/violated/not directly observable`，并给出整体 `feasible / not feasible`
...
Multimodal_input:
- keyframe_single(step_id=01, frame_index=002): <ITEM_DIR>/01_enter_the_kitchen_and_turn_on_the_light_to_illuminate_the_workspace/frame_002_ts_3.59s.jpg
Q: Step goal: "Enter the kitchen and turn on the light to illuminate the workspace." Given spatial precondition: "The hand is contacting the light_switch." Given affordance precondition: "The light_switch provides a pressable surface." Is this step physically feasible now? Answer in one English sentence, and state whether each given precondition is satisfied/violated/not directly observable in this frame.
A: It is feasible because the hand is visibly contacting the light_switch (spatial precondition satisfied) and the light_switch affords a pressable surface (affordance precondition satisfied).
```
...
- **任务说明**：解释跨步依赖：给定上一动作的一个具体后果（effect）与下一步的一个具体前置条件（precondition），输出“effect → precondition”的可解释链接（建议 1–2 句），避免任务退化为纯常识猜测。
- **字段（JSONPath）**：
  - 上下文：
    - `steps[i].step_goal`
    - `steps[i+1].step_goal`
    - `high_level_goal`（可选）
    - effect（从 `steps[i].causal_chain.causal_effect_on_spatial/affordance` 中拆分抽取 1 条原子陈述作为给定）
    - precondition（从 `steps[i+1].causal_chain.causal_precondition_on_spatial/affordance` 中拆分抽取 1 条原子陈述作为给定）
  - label：基于给定 effect 与 precondition 输出 1–2 句依赖解释（自由文本）
...
Multimodal_input:
- keyframe_single(step_id=01, frame_index=002): <ITEM_DIR>/01_enter_the_kitchen_and_turn_on_the_light_to_illuminate_the_workspace/frame_002_ts_3.59s.jpg
Q: Previous step goal: "Enter the kitchen and turn on the light to illuminate the workspace." Next step goal: "Retrieve a carrot and a cucumber from the refrigerator." Given effect: "The workspace becomes illuminated so objects are easier to see." Given precondition: "The person must be able to visually locate and access the refrigerator and vegetables safely." Explain how the effect satisfies the precondition.
A: Illumination increases visibility in the workspace, which directly satisfies the visual-access precondition needed to find and open the refrigerator to retrieve the vegetables.
```
...
- **字段（JSONPath）**：
  - 上下文：`high_level_goal`，`steps[i].step_goal`（前缀最后完成的 step_goal，用于定义“当前进度/下一步”）
  - 计划池（用于构造/校验坏计划）：`steps[*].step_goal`
  - 可选（用于构造“依赖违反”）：
...
Multimodal_input:
- video_prefix(prefix_end_step=02): <ITEM_DIR>/cumulative_last_frame_segments/segment_start_to_step02_last.mp4
Q: Context: High-level goal: "Prepare for cooking by turning on the light, gathering vegetables and tools, washing the vegetables, and chopping them on a cutting board." Last completed step (in this prefix): "Retrieve a carrot and a cucumber from the refrigerator." Based on this prefix, the following bad_plan_steps are proposed as the next steps: 1) "Slice the cucumber on the cutting board." 2) "Open the refrigerator and retrieve a carrot." 3) "Wash the vegetables under running water." Identify the flaw in the bad plan.
...
- **任务说明**：给定视频前缀与一个“只含单一扰动”的坏计划（bad_plan），输出纠正后的正确计划序列，用于训练失败反思中的“纠错→重规划”能力（与 Task_22 的缺陷定位配套）。
- **字段来源**：
  - 上下文：`high_level_goal`，`steps[i].step_goal`（前缀最后完成的 step_goal）
  - 输入：同 Task_22 的 `bad_plan_steps`
  - label：`steps[i+1:i+K].step_goal`
- **Multimodal_input 类型（四类）**：`video_prefix`
- **范例**：
...
Q: Context: High-level goal: "Prepare for cooking by turning on the light, gathering vegetables and tools, washing the vegetables, and chopping them on a cutting board." Last completed step (in this prefix): "Gather a cutting board and a knife and place them on the countertop." Based on this prefix, bad_plan_steps are proposed as the next steps: 1) "Wash the cucumber and carrot under running water and place them on the countertop." 2) "Put the vegetables back into the refrigerator and stop." 3) "Slice the cucumber into circular pieces on the cutting board." Repair the plan by outputting the corrected 3-step sequence.
...
- **字段（JSONPath）**：
  - 上下文：`steps[i].step_goal`
  - 输入（写入 Q 里）：`steps[i].counterfactual_challenge_question`
  - label：`steps[i].expected_challenge_outcome`
- **Multimodal_input 类型（四类）**：`keyframe_single`
- **范例**：
...
- **字段（JSONPath）**：
  - 上下文：`steps[i].step_goal`
  - 输入（写入 Q 里）：`steps[i].failure_reflecting.reason`
  - label：`steps[i].failure_reflecting.recovery_strategy`
- **Multimodal_input 类型（四类）**：`keyframe_single`
- **范例**：
...
- **字段来源**：
  - `steps[*].step_goal`（上下文：前缀最后完成的 step_goal，用于定义当前进度；写入 Q）
  - `high_level_goal`（可选；默认不提供，仅当需要用全局目标决定“继续/终止/切换子任务”时提供）
  - `steps[i].step_goal`（上下文：失败发生/需要继续完成的 step_goal）
...
Q: Context: Last completed step (in this prefix): "Wash the cucumber and carrot under running water and place them on the countertop." Failed step goal: "Slice the cucumber into circular pieces on the cutting board." Failure reason: "The cutting board is sliding during cutting." Recovery strategy: "Place a damp cloth under the cutting board to increase friction." After applying the recovery strategy, what is the most appropriate next step? Answer as a single step_goal.
