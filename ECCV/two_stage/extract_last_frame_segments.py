...
使用方式：
    python extract_last_frame_segments.py --video-output-dir <单个视频的输出目录>
...
def find_keyframe_path(video_dir: str, step_id: int, step_goal: str, frame_index: int, existing_path: Optional[str]) -> Optional[str]:
    # Prefer the path recorded in JSON if it still exists; also support relative paths
    # (e.g., when an item directory is moved to a different machine).
    if existing_path:
        resolved = _resolve_to_existing(existing_path, video_dir)
        if resolved:
            return resolved
    ...
    if candidates:
        return candidates[0]
    # Robust fallback: three-stage pipeline may truncate slug lengths; search by step_id prefix.
    for step_dir in sorted(glob(os.path.join(video_dir, f"{step_id:02d}_*"))):
        if not os.path.isdir(step_dir):
            continue
        candidates = sorted(glob(os.path.join(step_dir, f"frame_{frame_index:03d}_ts_*s.jpg")))
        if candidates:
            return candidates[0]
    return None
