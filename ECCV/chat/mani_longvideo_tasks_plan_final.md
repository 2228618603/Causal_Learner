...
- 每个 step 的关键帧图像：由 `steps[*].critical_frames[*].frame_index` 在对应 step 目录内解析 `frame_###_ts_XX.XXs.jpg`（脚本可能额外写入 `keyframe_image_path`，但任务构造不应依赖）
- 整个视频的均匀抽帧图像：`sampled_frames/`（默认 50 帧，`sample_###_ts_YY.YYs.jpg`）
- 两个Step间视频片段：相邻 step 的“尾关键帧”之间的 `last_frame_segments/*.mp4`（可用 `extract_last_frame_segments.py` 生成）
- 累积前缀长视频：从视频开始到每个 step 尾关键帧的 `cumulative_last_frame_segments/*.mp4`（可用 `extract_cumulative_last_frame_segments.py` 生成）
...
<item_dir>/
  ...
  causal_plan.json                      # 可选（两阶段常见；三阶段默认不产出）
  run_summary.json
  sampled_frames/
  ...
  01_<step_slug>/
    frame_007_ts_25.19s.jpg            # 关键帧（命名来自 frame_index + timestamp；三阶段中 frame_index 为 step-clip 局部序号，ts 为全局时间轴）
    ...
  02_<step_slug>/
    ...
```

补充：三阶段 item 目录中还会包含 `stage1/`、`stage2/` 等中间产物目录（抽帧 manifest、clips、prompt/raw 等），本文档的任务生成不依赖这些目录。
...
- **证据形态**：`video_clip`（`last_frame_segments/segment_step{i:02d}_to_step{i+1:02d}.mp4`）或其 `images_uniform_clip`
...
- **字段**：`high_level_goal`
- **证据形态**：`video_prefix`（`cumulative_last_frame_segments/segment_start_to_step{i:02d}_last.mp4`）或其抽帧序列
...
5) **一致性**：`meta.step_index/frame_index/segment_label` 必须能回溯到源数据（对三阶段：`frame_index` 回溯到对应 step 的 `critical_frames[*].frame_index`，并能在该 step 目录解析到 `frame_###_ts_*.jpg`；不要用它去索引根目录 `sampled_frames/`）。
...
- 输入（优先级从高到低）：
  ...
  2) 若 sampled_frames 缺失：用 `causal_plan_with_keyframes.json` 内所有 step 的关键帧作为“场景覆盖代理”（推荐用每个 step 的最早关键帧 `critical_frames[0]` 并通过 `frame_index` 在对应 step 目录解析 `frame_###_ts_*.jpg`）。
...
- **样本构造规则**：
  ...
  - 标签来自关键帧文件名的 `ts_XX.XXs`（跨 step 推荐）；`frame_index` 仅在同一 step 内可用。
...
- **证据来源（严格优先级）**：
  ...
  2) 若 sampled_frames 缺失：head 用 `steps[0].critical_frames[0]`，tail 用 `steps[-1].critical_frames[-1]`（通过 `frame_index` 在对应 step 目录解析 `frame_###_ts_*.jpg`）
- **样本构造规则**：
...
- 关键帧图片请按 `steps[i].critical_frames[j].frame_index` 在对应 step 目录解析 `frame_###_ts_*.jpg`（脚本可能写入 `keyframe_image_path`，但不作为依赖；示例统一使用 `<ITEM_DIR>/...` 相对路径）。
...
- Evidence（`images_uniform_scene`）：
  - 若无 `sampled_frames/`：可用每步最早关键帧做代理，例如：
    - `<ITEM_DIR>/01_enter_the_kitchen_and_turn_on_the_light_to_illuminate_the_workspace/frame_002_ts_3.59s.jpg`
    - `<ITEM_DIR>/02_retrieve_a_carrot_and_a_cucumber_from_the_refrigerator/frame_008_ts_25.19s.jpg`
    - `<ITEM_DIR>/03_gather_a_cutting_board_and_a_knife_and_place_them_on_the_countertop/frame_014_ts_46.80s.jpg`
    - `<ITEM_DIR>/04_wash_the_cucumber_and_carrot_under_running_water_and_place_them_on_the_countertop/frame_020_ts_68.39s.jpg`
- Q: Which stable objects are the task-relevant anchors for planning in this scene?
...
- Evidence（`keyframe_single`）：`<ITEM_DIR>/03_gather_a_cutting_board_and_a_knife_and_place_them_on_the_countertop/frame_014_ts_46.80s.jpg`
...
- Evidence（`keyframe_single`，step 尾帧）：`<ITEM_DIR>/04_wash_the_cucumber_and_carrot_under_running_water_and_place_them_on_the_countertop/frame_025_ts_86.39s.jpg`
